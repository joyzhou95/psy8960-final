...
## Drop NA values in the review columns and create a dataset containing only the reviews 
ion_reviews <- ion_tbl %>%
...
## Removed variables that had zero variance, as they would have zero predictive validity of the outcome
ion_tbl_final <- ion_tbl_com %>%
  select(-EmployeeCount, -Over18, -StandardHours)

## Create a dataset that does not contain any of the text variables
...
## as well as shorter computation time compared to the other two models
...
## Looking at the model comparison table, the elastic net model had the highest accuracy for predicting the test sample (.91) and the lowest accuracy
## for predicting the training sample (.89) compared to the other two models, and it also has the highest specificity (.99) and sensitivity (.40). 
...
## Including text data in the model increased specificity and predictive accuracy in the test set, yet the model performed
## worse in terms of predictive accuracy in the train set and sensitivity, which suggests that the text data probably added some 
## noise in the model so that it performs better in out-of-sample predictions yet worse with the training sample compared to the 
## model without text data. With all those being said, the difference between the two models is quite small. 
